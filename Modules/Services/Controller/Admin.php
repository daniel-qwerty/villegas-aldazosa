...
class Services_Controller_Admin extends Admin_Controller_Admin {
    ...
    public function Add() {
        ...
        if ($this->isPost()) {
            ...
            $icono = "";
            if (Com_File_Handler::getInstance()->setFile(get("Icono"))->hasErrors()) {
                Com_Wizard_Messages::getInstance()->addMessage(MESSAGE_INFORMATION, "El Archivo Seleccionado no pudo ser guardado por favor Intente Nuevamente");
            } else {
                if (!(Com_File_Handler::getInstance()->saveFile("Resources/Uploads/Image/"))) {
                    Com_Wizard_Messages::getInstance()->addMessage(MESSAGE_INFORMATION, "El Archivo Seleccionado no pudo ser guardado por favor Intente Nuevamente");
                } else {
                    $icono = Com_File_Handler::getInstance()->getFileName();
                }
            }

            $id = Services_Model_Service::getInstance()->doInsert($this->getPostObject(), $languages, $image, $icono);
    ...
    public function Edit() {
        ...
        if ($this->isPost()) {
            ...
            Services_Model_Service::getInstance()->doUpdate(get('id'), $this->getPostObject(), $image, $icono);

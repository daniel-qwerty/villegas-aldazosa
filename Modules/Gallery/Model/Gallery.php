...
class Gallery_Model_Gallery extends Com_Module_Model {
    ...
    public function getList($lanId, $type, $limit = 1000) {

        $text = new Entities_Gallery();
        return $text->getAll($text->getList()->where("GalLanId={$lanId} and GalStatus = 1 and GalType='{$type}' ")->limit(0, $limit));
    }

   

    public function getImage($lanId, $name) {
